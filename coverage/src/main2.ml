...
let get_loc { pexp_loc = { loc_start; loc_end; _ }; _ } =
  Format.asprintf "line %i, col %i to line %i col %i " loc_start.pos_lnum
    (loc_start.pos_cnum - loc_start.pos_bol)
    loc_end.pos_lnum
    (loc_end.pos_cnum - loc_end.pos_bol)

let get_mess sloc = apply_nolbl_s "Cover_runtime.increment" [ string_ sloc ]
...
let rec rewrite (expr : expression) : expression =
  let desc' =
    ...
    | Pexp_ifthenelse (condition, br_then, Some br_else) ->
        ...
        let newbr_then =
          seq pass_message (seq incr_message (rewrite br_then))
        in
        ...
        let newbr_else =
          seq pass_message (seq incr_message (rewrite br_else))
        in
    ...
    | Pexp_match (expression, case_list) ->
        let tracecase _cpt case =
          ...
          {
            case with
            pc_rhs = seq pass_message (seq incr_message (rewrite case.pc_rhs));
          }
        in
    ...
    | Pexp_fun (label, ex, pattern, expression) ->
        ...
        let new_expression =
          seq pass_message (seq incr_message (rewrite expression))
        in
    ...
    | Pexp_apply (expression, arg_list) ->
        (* semble être en redondance avec pexp_function *)
        ...
        (* let compiled_location1 = string_of_int  expression.pexp_loc.loc_start.pos_lnum  in
           ...
           let new_expression = seq incr_message (rewrite expression) in *)
        Pexp_apply (expression, arg_list)
    | Pexp_sequence (e1, e2) -> Pexp_sequence (rewrite e1, rewrite e2)
    | Pexp_function _ (* on ne l'utilisera presque pas*) | Pexp_tuple _
    | Pexp_variant (_, _)
    | Pexp_record (_, _)
    | Pexp_field (_, _)
    | Pexp_setfield (_, _, _)
    ...
    | Pexp_letop _ | Pexp_extension _ | Pexp_unreachable ->
        Format.asprintf "%a: not implemented yet - skipping" print_expression
          expr
        |> failwith
  in
  { expr with pexp_desc = desc' }

let float_ x = Exp.constant (Pconst_float (string_of_float x, None))
let ( +@ ) e1 e2 = apply_nolbl_s "+." [ e1; e2 ]

let ( *@ ) e1 e2 =
  (*erreur si je mets * ou x *)
  apply_nolbl_s "*." [ e1; e2 ]

let ( /@ ) e1 e2 = apply_nolbl_s "/." [ e1; e2 ]
let ( =@ ) e1 e2 = apply_nolbl_s "=." [ e1; e2 ]
let case_to_exp case = case.pc_rhs

let visit_exp exp =
  apply_nolbl_s "Cover_runtime.isVisited_f" [ string_ (get_loc exp) ]

let prod_vexp loc exp = loc *@ exp
...
let rec couverture expression =
  ...
  | Pexp_match (_expression, case_list) ->
      ...
      let div_length exp =
        exp /@ float_ (float_of_int (List.length case_list))
      in
      let couv_recu exp =
        couverture (apply_nolbl_s "Cover_runtime.get" [ string_ (get_loc exp) ])
      in
      let list_avec_recu =
        List.map couv_recu (List.map case_to_exp case_list)
      in
      let couv_quotient = List.map div_length list_couv in
      let prod_recu = List.map2 prod_vexp list_avec_recu couv_quotient in
      let couv_quotient_visited = List.map2 prod_vexp locs prod_recu in
      let rec sum l = match l with [] -> float_ 0.0 | h :: t -> h +@ sum t in
      sum couv_quotient_visited
      (* | Pexp_let (flag, vb, expression) ->  *)
  | Pexp_fun (_label, _ex, _pattern, expression) ->
      propagate expression 1.0
      (* | Pexp_ifthenelse (condition, branch_then, None) ->   *)
  | _ -> float_ 1.0
...
(* ne reconnait pas () et donc multiplie string et float*)
...
let mapper =
  ...
  let handle_str mapper (str : structure) =
    ...
    aux [] str
  in
  { default_mapper with value_binding = handle_bind; structure = handle_str }

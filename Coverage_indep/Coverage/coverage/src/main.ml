...
let cover_function vb  =
  ...
  let add_cov_table  vb =    
      let name = get_name vb in
      let get_cov_vb = Vb.mk (Pat.var (def_loc ("()"))) (apply_nolbl_s "Testify_runtime.add"  [string_ name ;Exp.fun_ Nolabel None
      (Pat.construct (lid_loc "()") None)
...
let mapper =
  let handle_bind _mapper (bind:value_binding) =
         {bind with pvb_expr = rewrite (bind.pvb_expr )} in
  ...
  let handle_str mapper (str:structure) =  
    let rec aux acc str =
      ...
      | ({pstr_desc= Pstr_value (_, [pvb]); _} as h) :: tl ->
         let _get_cov = cover_function pvb in
         let h' = mapper.structure_item mapper h in
         aux  (((add_cov_table pvb )::((h' :: acc)))) tl
      | h :: tl ->
          let h' = mapper.structure_item mapper h in
          aux (h' :: acc) tl   
    in 
